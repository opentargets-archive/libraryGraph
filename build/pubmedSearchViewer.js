...
var _network = require('./src/network.js');

var _network2 = _interopRequireDefault(_network);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = _network2.default;

},{"./src/network.js":36}],3:[function(require,module,exports){
...
},{"./src/api.js":34}],34:[function(require,module,exports){
...
exports.toggleTerminals = toggleTerminals;
exports.highlightCluster = highlightCluster;
...
function getQuery(query) {
    ...
    var q = { "query": { "query_string": { "query": "braf" } }, "controls": { "use_significance": true, "sample_size": 2000, "timeout": 5000 }, "connections": { "vertices": [{ "field": "abstract", "size": 5, "min_doc_count": 3 }] }, "vertices": [{ "field": "abstract", "size": 5, "min_doc_count": 3 }] };

    // const url = 'https://core:PubMine@2017@62d68b658f9e146c7b58491bd01ea91b.eu-west-1.aws.found.io:9243/api/graph/graphExplore';
    var url = 'https://62d68b658f9e146c7b58491bd01ea91b.eu-west-1.aws.found.io:9243/!publication-data/_xpack/_graph/_explore';
    // return http.post({
    //     url: url,
    //     body: q,
    //     headers: {
    //         Authorization: `Basic Y29yZTpQdWJNaW5lQDIwMTc=`
    //     }
    // });
    var ax = _axios2.default.create({
        timeout: 30000,
        headers: {
            "Authorization": 'Basic Y29yZTpQdWJNaW5lQDIwMTc=' // base64(core:PubMine2017)
        }
    });
...
function toggleTerminals() {
    ...
    var _nodes$getAll = nodes.getAll({ terminals: config.terminals }),
        allNodes = _nodes$getAll.nodes,
        allLinks = _nodes$getAll.links;
...
function highlightCluster(nodeId) {
    var _allNodes = nodes.getNodes();
    ...
    if (!nodeId) {
        ...
        try {
            for (var _iterator = _allNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        try {
            for (var _iterator2 = _allNodes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                var _node2 = _step2.value;

                _node2.visible = false;
            }
        ...
        var _node = nodes.getNode(nodeId);
        // It may be a clustered node, if so, take the first outLink...
        if (!_node) {
            _node = nodes.getNode(nodeId.split('-')[0]);
        }

        var disease1 = nodes.getNode(config.efo1);
        var disease2 = nodes.getNode(config.efo2);
        var cluster = nodes.getCluster(_node, [disease1, disease2]);
    ...
    var _nodes$getAll2 = nodes.getAll({ terminals: config.terminals }),
        allNodes = _nodes$getAll2.nodes,
        allLinks = _nodes$getAll2.links;
...
function getInteractors(uniprotId) {
    var url = 'http://www.omnipathdb.org/interactions/' + uniprotId + '?format=json&fields=sources';
...
},{"./nodes.js":37,"axios":3}],36:[function(require,module,exports){
'use strict';
...
exports.default = function () {
    ...
    var render = function render(container) {
        (0, _data.getQuery)(config.query).then(function (resp) {
            console.log(resp);
    ...
    var api = (0, _tnt2.default)(render).getset(config);
...
var config = {
    ...
    height: 800,
    nodeSize: 5,
    colorScale: d3.scaleLinear().range([d3.rgb("#007AFF"), d3.rgb('#FFF500')]) // The domain is set dynamically
};
...
function update() {
    ...
    link.exit().remove();
    link.enter().append("line").attr("stroke-width", function () {
        return 1;
    }).style({ "stroke": "gray" });
    ...
    node.exit().remove();
    node.enter().append("circle").each(function (d) {
        console.log('new node created');
    }).attr("cx", function (d) {
        return d.x;
    }).attr("cy", function (d) {
        return d.y;
    }).attr("r", config.nodeSize).style("stroke", function (d, i) {
        return "black";
        ...
        return "green";
    }).call(force.drag).on("mousedown", function () {
...
function getWeightExtent(config) {
    ...
    var extent = d3.extent(links, function (d) {
        return d.weight;
    });
    return d3.scaleLinear().domain(extent).range([1, config.radius * 0.7]);
}

function createForce(container, config) {

    var meter = d3.select(container).append("div").attr("class", "interactions-network-progress").node();

    weightScale = getWeightExtent(config);

    var canvas = d3.select(container).append("canvas").attr("width", config.width).attr("height", config.height);
    ...
    for (var k = 2; k < data.nodes.length; k++) {
    ...
    .force("collision", d3.forceCollide().radius(30)).force("links", d3.forceLink(data.links)).stop();

    // .on("tick", ticked)
    // .on("end", ended);

    canvas.call(d3.drag().container(canvas.node()).subject(dragsubject).on("drag", dragged)).call(d3.zoom().scaleExtent([0.5, 5]).on("zoom", zoomed));
    ...
    function dragsubject() {
        ...
        var thisNode = force.find(x, y, config.radius);
        if (!thisNode) return;
...
},{"./data.js":35,"tnt.api":33}],37:[function(require,module,exports){
'use strict';
...
// Setters
nodesMap.addNode = function (_ref) {
    var id = _ref.id,
        label = _ref.label,
        _ref$fixed = _ref.fixed,
        fixed = _ref$fixed === undefined ? false : _ref$fixed,
        _ref$type = _ref.type,
        type = _ref$type === undefined ? "target" : _ref$type;
...
nodesMap.addLink = function (source, target) {
    // source and targets are ids
    // Get the source
    var n1 = nodesMap.nodes.get(source);
    var n2 = nodesMap.nodes.get(target);
...
function visitCluster(cluster, visited, toVisit, dest) {
    // dest is an array of destination nodes
    ...
    try {
        for (var _iterator = dest[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            var d = _step.value;
    ...
    try {
        for (var _iterator2 = node.outLinks[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            var nextNode = _step2.value;
    ...
    try {
        for (var _iterator3 = node.inLinks[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            var _nextNode = _step3.value;

            toVisit.push(_nextNode);
        }
    } catch (err) {
        _didIteratorError3 = true;
        _iteratorError3 = err;
...
nodesMap.joinTerminals = function () {
    ...
    try {
        for (var _iterator4 = nodesMap.nodes[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            var _step4$value = _slicedToArray(_step4.value, 2),
                nodeName = _step4$value[0],
                node = _step4$value[1];

            if (node.outLinks.size === 1 && node.inLinks.size === 0) {
                node.isSingleton = true;
                var target = Array.from(node.outLinks.values())[0];
...
nodesMap.getAll = function (_ref2) {
    var _ref2$terminals = _ref2.terminals,
        terminals = _ref2$terminals === undefined ? false : _ref2$terminals;
    ...
    try {
        for (var _iterator5 = nodesMap.nodes[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
    ...
    try {
        for (var _iterator6 = all.nodes[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
            var sourceNode = _step6.value;
            ...
            try {
                for (var _iterator7 = sourceNode.outLinks[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
                    var _target = _step7.value;

                    all.links.push({
                        source: source,
                        target: indexes.get(_target.id)
                    });
                }
            } catch (err) {
...
//# sourceMappingURL=pubmedSearchViewer.js.map
